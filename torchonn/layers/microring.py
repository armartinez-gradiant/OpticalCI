...
class MicroringResonator(nn.Module):
    ...
    def __init__(
        ...
        coupling_strength: float = 0.08,  # Acoplamiento
        q_factor: float = 30000,  # Factor Q
    ...
    def validate_critical_coupling(self) -> Dict[str, float]:
        """Validar si estamos cerca de critical coupling."""
        kappa = float(self.coupling_tuning.data.item())
        alpha = float(torch.exp(torch.tensor(-np.pi / self.q_factor)).item())
        
        # Critical coupling ideal: κ ≈ √(1 - α²)
        kappa_critical = np.sqrt(1 - alpha**2)
        coupling_ratio = kappa / kappa_critical
        
        # Extinction ratio teórico
        r = np.sqrt(1 - kappa**2)
        transmission_min = abs((r - alpha) / (1 - alpha * r))**2
        transmission_max = abs((r + alpha) / (1 + alpha * r))**2
        extinction_ratio_db = 10 * np.log10(transmission_max / transmission_min)
        
        return {
            'kappa_actual': kappa,
            'kappa_critical': kappa_critical,
            'coupling_ratio': coupling_ratio,  # 1.0 = ideal
            'extinction_ratio_db': extinction_ratio_db,
            'regime': 'under' if coupling_ratio < 0.8 else 'critical' if coupling_ratio < 1.2 else 'over'
        }

    def print_debug_info(self):
        """Imprimir información de debug del microring."""
        print(f"Debug Microring:")
        print(f"  Radius: {self.radius*1e6:.1f} μm")
        print(f"  Coupling: {self.coupling_strength:.3f}")
        print(f"  Q factor: {self.q_factor}")
        print(f"  Center wavelength: {self.center_wavelength*1e9:.1f} nm")
        print(f"  FSR: {self.fsr*1e12:.1f} pm")
        print(f"  Expected ER: ~{10*np.log10(4*self.q_factor*self.coupling_strength**2):.1f} dB")
