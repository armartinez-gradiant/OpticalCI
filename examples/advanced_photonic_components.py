...
class AddDropMRR(nn.Module):
    ...
    def __init__(self,
        radius: float = 10e-6,
        coupling_strength_1: float = 0.1  # 🔧 FIXED: Lower for better extinction,  # Input-ring coupling
        coupling_strength_2: float = 0.1  # 🔧 FIXED: Lower for better extinction,  # Ring-drop coupling  
        q_factor: float = 15000,
        center_wavelength: float = 1550e-9,
        n_eff: float = 2.4,  # Effective index
        n_g: float = 4.2,    # 🔧 FIX: Added group index
        device: Optional[torch.device] = None,,
        **kwargs
    ):
        super().__init__()
        ...
        self.fsr = center_wavelength**2 / (4.2 * self.circumference)  # n_g = 4.2 for Si

        # Round-trip loss (del Q factor)
        loss_per_round_trip = 2 * np.pi / q_factor  # 🔧 FIXED: More realistic
        self.register_buffer("alpha", torch.tensor(np.exp(-loss_per_round_trip/2), device=device))  # 🔧 FIXED  # Amplitude transmission
